...
cc_security_panel_update_password_toggle (CcSecurityPanel *panel)
{
        ...
        if (priv->toggle_id)
                g_signal_handler_unblock (priv->password_toggle,
                                          priv->toggle_id);
...
{
        CcSecurityPanelPrivate *priv = GET_PRIVATE (panel);
        GtkWidget *box, *toggle_box, *c_area, *label;
        GtkBuilder *builder;
        GError *error = NULL;
        gchar *objects[] = {"vbox1"};
        ...
        gtk_container_add (GTK_CONTAINER (panel),GET_WIDGET (builder, "vbox1"));

        toggle_box = GET_WIDGET (builder, "password_toggle_box");
        priv->password_toggle = mx_gtk_light_switch_new ();
        gtk_widget_show (priv->password_toggle);
...
cc_security_panel_dispose (GObject *object)
{
        ...
        if (priv->builder) {
                ...
                priv->current_info_bar = NULL;
                priv->new_entry = NULL;
                priv->verify_entry = NULL;
                priv->verify_info_bar = NULL;
                priv->verify_info_label = NULL;
                priv->save_button = NULL;
                priv->password_toggle = NULL;
        }

        G_OBJECT_CLASS (cc_security_panel_parent_class)->dispose (object);
}
